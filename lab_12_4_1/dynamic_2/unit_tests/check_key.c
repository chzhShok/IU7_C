...
typedef int (*mykey_t)(const int *pb_src, const int *pe_src, int **pb_dst, int **pe_dst);
...
static void load_library(const char *libname)
{
    ...
    key = (mykey_t) (uintptr_t) dlsym(lib, "key");
}
